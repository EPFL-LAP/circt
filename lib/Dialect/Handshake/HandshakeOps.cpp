...
void LSQOp::build(OpBuilder &builder, OperationState &odsState, Value memref,
                  ArrayRef<std::pair<Value, ArrayRef<Operation *>>> accesses,
                  int id) {
  // Add memref operand
  odsState.addOperands(memref);

  SmallVector<LSQBlockAttr> lsqBlocks;
  unsigned ldCount = 0;

  // Go through the list of memory accesses block and block and construct the
  // LSQ inputs/attributes
  for (auto &[ctrlVal, blockOps] : accesses) {
    odsState.addOperands(ctrlVal);
    for (auto &op : blockOps) {
      if (auto loadOp = dyn_cast<handshake::DynamaticLoadOp>(op)) {
        odsState.addOperands(loadOp.getAddressResult());
        ldCount++;
      } else {
        auto storeOp = dyn_cast<handshake::DynamaticStoreOp>(op);
        assert(storeOp && "input operation must either be load or store");
        odsState.addOperands(storeOp.getResults());
      }
    }
  }
  ...
  odsState.types.push_back(builder.getNoneType());

  // Memory interface attributes
  Type i32Type = builder.getIntegerType(32);
  odsState.addAttribute("accesses", builder.getAttr<LSQBlocksAttr>(
                                        odsState.getContext(), lsqBlocks));
  odsState.addAttribute("id", builder.getIntegerAttr(i32Type, id));
}

LogicalResult LSQOp::verify() { return emitOpError() << "unimplemented"; }
